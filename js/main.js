...
if(thing =  true ){
	console.log('thing, i have no idea what it is', this); 
...
(function(){ 
	...
	window.projector2 = new THREE.Projector(); 
	renderer2.setSize( window.innerWidth, window.innerHeight ); 
	document.body.appendChild( renderer2.domElement ); 
	$(renderer2.domElement ).css({'width':'100%', 'height':'100%'}); 
	...
	function render() {
		...
		//projector.projectScene(scene, camera, true); 
...
window.onload	= function(){ 
	...
	var onCreate	= function(event){ 
		...
		if( true ){
			...
			var material = new THREE.MeshLambertMaterial({color: 0xFF00AA}); 
			var mesh = new THREE.Mesh(geometry, material); 
	...
	var onUpdate	= function(event){ 
		...
		window.tm = event.matrix;  
		//meshC.matrix = event.matrix; 
		//meshC.matrix.copy(event.matrix); 
